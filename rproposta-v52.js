/* R-Proposta Digital v5.2 — ADM sem bloqueio de validação e acesso administrativo discreto. */
(() => {
  if (window.__rpropV52Index) return;
  window.__rpropV52Index = true;
  ...
  window.__rpropV52Adm = { isActive: () => admActive };
  ...
  // ----- Draft: 2 hours from last saved change -----
  function readDraft(){
    try{const raw=localStorage.getItem(DRAFT_KEY);if(!raw)return null;const x=JSON.parse(raw);if(!x||!x.lastChanged||Date.now()-Number(x.lastChanged)>DRAFT_TTL){localStorage.removeItem(DRAFT_KEY);return null;}return x;}catch(e){return null;}
  }
  window.__rpropV51ClearDraft=()=>{try{localStorage.removeItem(DRAFT_KEY);}catch(e){}};

  async function captureDraft(){
    if(draftRestoring || editorIsOpen())return;
    const seq=normalSeq(); if(!seq.length)return;
    const docs={};
    for(const it of seq){
      try{const r=await postFrame(it.iframe,{type:'EDITOR_EXPORT_V342'},3500);if(!r||!r.ok)continue;(docs[it.docId]||(docs[it.docId]=[])).push({inst:Number(it.inst||1),controls:Array.isArray(r.controls)?r.controls:[]});}catch(e){}
    }
  ...
  function scheduleDraft(){if(draftRestoring||editorIsOpen())return;clearTimeout(saveTimer);saveTimer=setTimeout(captureDraft,420);}

  async function applyCounts(counts){
    const m=main();if(!m)return;
    for(const d of m.DOCS){const wanted=Math.max(d.min,Math.min(d.max,Number(counts&&counts[d.id]||d.min)));m.state.counts[d.id]=wanted;m.state.activeInstIndex[d.id]=Math.min(wanted,Math.max(1,m.state.activeInstIndex[d.id]||1));m.ensureFrames(d.id);m.pruneFrames(d.id);}m.saveState(m.state);m.activateDoc(m.state.activeDocId,m.state.activeInstIndex[m.state.activeDocId]);
  }

  async function restoreDraft(draft){
    draftRestoring=true; draftOverlay.classList.remove('open');
    try{
      await applyCounts(draft.counts||{}); await sleep(400);
      for(const docId of Object.keys(draft.docs||{})){
        for(const entry of draft.docs[docId]||[]){
          const wrap=document.querySelector('.framewrap[data-doc="'+docId+'"][data-inst="'+String(entry.inst||1)+'"]');const iframe=wrap&&wrap.querySelector('iframe');if(!iframe)continue;
          for(let attempt=0;attempt<3;attempt++){try{const r=await postFrame(iframe,{type:'EDITOR_IMPORT_V342',controls:entry.controls||[]},9000);if(r&&r.ok)break;}catch(e){await sleep(300);}}
        }
      }
      await sleep(250); scheduleStatus(50);
    }finally{draftRestoring=false;}
  }

  function checkDraftOnLoad(){const d=readDraft();if(d)draftOverlay.classList.add('open');}
  document.getElementById('v51DraftRecover').onclick=()=>{const d=readDraft();if(d)restoreDraft(d);else draftOverlay.classList.remove('open');};
  document.getElementById('v51DraftDiscard').onclick=()=>{window.__rpropV51ClearDraft();try{localStorage.removeItem('sistema_unico_docs_v7');}catch(e){}location.reload();};

  window.addEventListener('message',e=>{const m=e.data;if(!m||m.__rpropV51!==true)return;if(m.type==='STATE_CHANGED'){if(!editorIsOpen()){scheduleDraft();scheduleStatus();}}},false);
  document.addEventListener('click',e=>{if(e.target&&e.target.closest&&e.target.closest('.instbtn.add,.instbtn.danger')){setTimeout(()=>{scheduleDraft();scheduleStatus();if(admActive&&admOrigin==='normal')setAdmOnNormalFrames(true);},250);}},true);
  ...
  installAdmButtons();
  setTimeout(()=>{checkDraftOnLoad();scheduleStatus(100);},900);
})();
